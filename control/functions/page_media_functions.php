...
<?php } ?>
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <input type="submit" class="btn btn-primary col-6" name="AddSubmit" id="AddSubmit" value="Add Media" />
      </form>
    </div>  <?php } ?>
  </div>
  ...
  foreach(array_filter(glob('../uploads'.'/*'),'is_file') as $file)
  {
    ...
    else if (strcasecmp(substr($file,-4),'.png') == 0 || strcasecmp(substr($file,-4),'.jpg') == 0 || strcasecmp(substr($file,-5),'.jpeg') == 0 || strcasecmp(substr($file,-4),'.bmp') == 0 || strcasecmp(substr($file,-4),'.gif') == 0 || strcasecmp(substr($file,-5),'.tiff') == 0)
    {
      ...
      else
      {
        ...
        echo '<p class="btn btn-primary btn-sm" onclick="copier(\'' . $count . '\')">Copy Link</p></td><td>&ltimg src=&quot;' . PROTOCOL . URL . substr($file,2) . '&quot; alt=&quot;' . substr($file,11) . '&quot; /&gt;</td><td>' . substr($file,2) . '</td>';
      }
      echo '</tr>';
    }
    else if (strcasecmp(substr($file,-4),'.mp4') == 0 || strcasecmp(substr($file,-5),'.webm') == 0 || strcasecmp(substr($file,-4),'.ogv') == 0)
    {
      ...
      if ($pageOrPlugin != 'Plugin')
      {
        echo '<td><form method="post" style="display:inline;"><input id="Delete" name="Delete" type="hidden" value="' . $file . '" /><input type="submit" class="btn btn-primary btn-sm" name="DeleteSubmit" value="Delete" /></form></td>';
      }
      echo '</tr>';
    }
